...
switch ($mode) {
	case "ContactForm": 
            ...
            $restrictedword=array("lund","gand","fuck","fucking","sex","sex chat","porn","prostitute","sexual","blowjob","handjob");
            $flag=false;
            foreach($restrictedword as $word){               
                if (strpos(strtolower($comments), $word) !== false) {
                    $flag=true;
                }             
            } 
			...
			$message .= "Email: <a href='mailto:".$UserEmail."'>".$UserEmail."</a><br>";
			$message .= "Mobile: +91  ".$UserMobile."<br>";
			$message .= "Enquiry:  ".$comments."<br>";
			$message .= "Date: From ".$FromDate." To ".$ToDate;
			$message .= "		<br></p>";
			...
			$message_customer = '<p>Thank you for your message.</p><p>A Brunton Boatyard concierge will call or email you back at the earliest</p>';
			
			//echo $message;
			
			//$subject = "CGH Earth Property Booking Request through Campaign- ".time();
			
			$subject = "Brunton Boatyard Enquiry  ".time();
			...
			$headers .= 'From: Brunton Boatyard <webenquiry@cghearth.com>' . "\r\n";
			...
			('$UserName','$UserEmail','$UserMobile','$comments','$FromDate','$ToDate','brunton/offers')";
			if(!mysqli_query($bd,$qry)){
				echo mysqli_error($bd);
			} 
			
			try {
                if($flag){                                                          @mail('digital@stirredcreative.in,sanjana.s@stirredcreative.in,faustina@stirredcreative.in,shriya@stirredcreative.in,elvina@stirredcreative.in,ijas@stirredcreative.in', $subject, $message, $headers); 
                    @mail('bhabani@stirredcreative.in', $subject, $message, $headers); 
                }else{
                    @mail('directsales@cghearth.com, directsales4@cghearth.com, feedback@cghearth.com, digital@stirredcreative.in, faustina@stirredcreative.in, shriya@stirredcreative.in, elvina@stirredcreative.in,ijas@stirredcreative.in, bhabani@stirredcreative.in', $subject, $message, $headers);
                    @mail($UserEmail, $subject, $message_customer, $headers);
                }
				echo "<p>Thank you for your message.</p><p>A Brunton Boatyard concierge will call or email you back at the earliest</p> ";
